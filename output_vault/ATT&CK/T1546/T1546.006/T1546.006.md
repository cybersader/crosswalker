...
desc: "|-\n  Adversaries may establish persistence by executing malicious content\
  ...
  \ and dependencies.(Citation: Writing Bad Malware for OSX) There are tools available\
  \ to perform these changes.\n  \n  Adversaries may modify Mach-O binary headers\
  \ to load and execute malicious dylibs every time the binary is executed. Although\
  \ any changes will invalidate digital signatures on binaries because the binary\
  \ is being modified, this can be remediated by simply removing the LC_CODE_SIGNATURE\
  \ command from the binary so that the signature isn't checked at load time.(Citation:\
