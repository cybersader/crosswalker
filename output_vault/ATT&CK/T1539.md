...
desc: "|-\n  An adversary may steal web application or service session cookies and\
  \ use them to gain access to web applications or Internet services as an authenticated\
  ...
  \ is not actively used. Cookies can be found on disk, in the process memory of the\
  \ browser, and in network traffic to remote systems. Additionally, other applications\
  ...
  \  \n  There are several examples of malware targeting cookies from web browsers\
  \ on the local system.(Citation: Kaspersky TajMahal April 2019)(Citation: Unit 42\
  ...
  \ also open source frameworks such as `Evilginx2` and `Muraena` that can gather\
  \ session cookies through a malicious proxy (e.g., [Adversary-in-the-Middle](https://attack.mitre.org/techniques/T1557))\
  \ that can be set up by an adversary and used in phishing campaigns.(Citation: Github\
  \ evilginx2)(Citation: GitHub Mauraena)\n  \n  After an adversary acquires a valid\
  \ cookie, they can then perform a [Web Session Cookie](https://attack.mitre.org/techniques/T1550/004)\
