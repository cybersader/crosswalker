...
desc: "|-\n  Adversaries may gather information about the victim's network trust dependencies\
  ...
  \ managed service providers, contractors, etc.) that have connected (and potentially\
  \ elevated) network access.\n  \n  Adversaries may gather this information in various\
  \ ways, such as direct elicitation via [Phishing for Information](https://attack.mitre.org/techniques/T1598).\
  \ Information about network trusts may also be exposed to adversaries via online\
  \ or other accessible data sets (ex: [Search Open Technical Databases](https://attack.mitre.org/techniques/T1596)).(Citation:\
