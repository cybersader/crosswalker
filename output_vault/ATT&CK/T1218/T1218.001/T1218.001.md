...
desc: "|-\n  Adversaries may abuse Compiled HTML files (.chm) to conceal malicious\
  ...
  \ using underlying components of the Internet Explorer browser (Citation: Microsoft\
  \ HTML Help ActiveX) loaded by the HTML Help executable program (hh.exe). (Citation:\
  \ Microsoft HTML Help Executable Program)\n  \n  A custom CHM file containing embedded\
  \ payloads could be delivered to a victim then triggered by [User Execution](https://attack.mitre.org/techniques/T1204).\
