...
desc: "|-\n  Adversaries may use SID-History Injection to escalate privileges and\
  \ bypass access controls. The Windows security identifier (SID) is a unique value\
  \ that identifies a user or group account. SIDs are used by Windows security in\
  \ both security descriptors and access tokens. (Citation: Microsoft SID) An account\
  \ can hold additional SIDs in the SID-History Active Directory attribute (Citation:\
  \ Microsoft SID-History Attribute), allowing inter-operable account migration between\
  \ domains (e.g., all values in SID-History are included in access tokens).\n  \n\
