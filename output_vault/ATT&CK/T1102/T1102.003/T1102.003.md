...
desc: "|-\n  Adversaries may use an existing, legitimate external Web service as a\
  \ means for sending commands to a compromised system without receiving return output\
  ...
  \ systems may return no output at all in cases where adversaries want to send instructions\
  \ to systems and do not want a response.\n  \n  Popular websites and social media\
  ...
  \ to a compromise. Using common services, such as those offered by Google or Twitter,\
  \ makes it easier for adversaries to hide in expected noise. Web service providers\
