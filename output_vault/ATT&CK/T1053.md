...
desc: "|-\n  Adversaries may abuse task scheduling functionality to facilitate initial\
  ...
  \ startup or on a scheduled basis for persistence. These mechanisms can also be\
  \ abused to run a process under the context of a specified account (such as one\
  \ with elevated permissions/privileges). Similar to [System Binary Proxy Execution](https://attack.mitre.org/techniques/T1218),\
