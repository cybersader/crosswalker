...
desc: "|-\n  Adversaries with no prior knowledge of legitimate credentials within\
  ...
  \ or may not take into account the target's policies on password complexity or use\
  \ policies that may lock accounts out after a number of failed attempts.\n  \n \
  ...
  \ (Citation: Cylance Cleaver)\n  \n  Typically, management services over commonly\
  ...
  \ services, adversaries may \"target single sign-on (SSO) and cloud-based applications\
  \ utilizing federated authentication protocols,\" as well as externally facing email\
  \ applications, such as Office 365.(Citation: US-CERT TA18-068A 2018). Further,\
  \ adversaries may abuse network device interfaces (such as `wlanAPI`) to brute force\
  \ accessible wifi-router(s) via wireless authentication protocols.(Citation: Trend\
  \ Micro Emotet 2020)\n  \n  In default environments, LDAP and Kerberos connection\
  \ attempts are less likely to trigger events over SMB, which creates Windows \"\
