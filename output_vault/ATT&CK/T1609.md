...
desc: "|-\n  Adversaries may abuse a container administration service to execute commands\
  ...
  \ within an environment.(Citation: Docker Daemon CLI)(Citation: Kubernetes API)(Citation:\
  \ Kubernetes Kubelet)\n  \n  In Docker, adversaries may specify an entrypoint during\
  \ container deployment that executes a script or command, or they may use a command\
  \ such as <code>docker exec</code> to execute a command within a running container.(Citation:\
