...
desc: "|-\n  Adversaries may execute malicious payloads via loading shared modules.\
  ...
  \ on a victim system. For example, adversaries can modularize functionality of their\
  \ malware into shared objects that perform various functions such as managing C2\
  \ network communications or execution of specific actions on objective.\n  \n  The\
  \ Linux & macOS module loader can load and execute shared objects from arbitrary\
  ...
  \ Windows module loader can be instructed to load DLLs from arbitrary local paths\
  \ and arbitrary Universal Naming Convention (UNC) network paths. This functionality\
