...
desc: "|-\n  Adversaries may reduce the level of effort required to decrypt data transmitted\
  \ over the network by reducing the cipher strength of encrypted communications.(Citation:\
  \ Cisco Synful Knock Evolution)\n  \n  Adversaries can weaken the encryption software\
  \ on a compromised network device by reducing the key size used by the software\
  \ to convert plaintext to ciphertext (e.g., from hundreds or thousands of bytes\
  \ to just a couple of bytes). As a result, adversaries dramatically reduce the amount\
  \ of effort needed to decrypt the protected information without the key.\n  \n \
