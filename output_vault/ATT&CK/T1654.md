...
desc: "|-\n  Adversaries may enumerate system and service logs to find useful data.\
  \ These logs may highlight various types of valuable insights for an adversary,\
  ...
  \ such as the Azure VM Agent's `CollectGuestLogs.exe` to collect security logs from\
  \ cloud hosted infrastructure.(Citation: SIM Swapping and Abuse of the Microsoft\
  \ Azure Serial Console)\n  \n  Adversaries may also target centralized logging infrastructure\
  \ such as SIEMs. Logs may also be bulk exported and sent to adversary-controlled\
  \ infrastructure for offline analysis.\n  \n  In addition to gaining a better understanding\
  \ of the environment, adversaries may also monitor logs in real time to track incident\
  \ response procedures. This may allow them to adjust their techniques in order to\
  \ maintain persistence or evade defenses.(Citation: Permiso GUI-Vil 2023)"
---
