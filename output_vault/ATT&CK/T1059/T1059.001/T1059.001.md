...
desc: "|-\n  Adversaries may abuse PowerShell commands and scripts for execution.\
  ...
  \ may also be used to download and run executables from the Internet, which can\
  \ be executed from disk or in memory without touching disk.\n  \n  A number of PowerShell-based\
  \ offensive testing tools are available, including [Empire](https://attack.mitre.org/software/S0363),\
  \  [PowerSploit](https://attack.mitre.org/software/S0194), [PoshC2](https://attack.mitre.org/software/S0378),\
  \ and PSAttack.(Citation: Github PSAttack)\n  \n  PowerShell commands/scripts can\
  \ also be executed without directly invoking the <code>powershell.exe</code> binary\
  \ through interfaces to PowerShell's underlying <code>System.Management.Automation</code>\
  \ assembly DLL exposed through the .NET framework and Windows Common Language Interface\
