...
desc: "|-\n  Adversaries may upload malware to third-party or adversary controlled\
  ...
  \ other malicious content. Adversaries may upload malware to support their operations,\
  \ such as making a payload available to a victim network to enable [Ingress Tool\
  \ Transfer](https://attack.mitre.org/techniques/T1105) by placing it on an Internet\
  \ accessible web server.\n  \n  Malware may be placed on infrastructure that was\
  \ previously purchased/rented by the adversary ([Acquire Infrastructure](https://attack.mitre.org/techniques/T1583))\
  ...
  \ files, such as application binaries, virtual machine images, or container images,\
  \ to third-party software stores or repositories (ex: GitHub, CNET, AWS Community\
  \ AMIs, Docker Hub). By chance encounter, victims may directly download/install\
  \ these backdoored files via [User Execution](https://attack.mitre.org/techniques/T1204).\
