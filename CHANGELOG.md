...
- Stack setup now chooses nested NIST 800-53 and CRI Profile recipes without changing the flat recipes proposed by ordinary file recognition. Controls and CRI upper levels become folder notes, while enhancements and diagnostic statements become nested notes. The Detail control can exclude enhancements and diagnostic statements with a run-scoped row filter; the canonical recipes stay unchanged. New stack notes record the bundled recipe ID; refresh offers require both that ID and the same recorded recipe hash, including the detail filter. Stack refresh offers are withheld for older custom IDs or ambiguous detail; start a new set unless the exact earlier recipe can be restored. NIST families are folders only: family-level mapping links remain unresolved until a separate identity-safe family-note design. No licensed framework rows or prose are bundled. Source: `recipes/import/nist-800-53-nested.json`, `recipes/import/cri-profile-v2-2-nested.json`, `src/import/stack/`.
