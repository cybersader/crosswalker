...
- A re-run defaults unchanged, recorded slots to a write-free Skip. Changed sources default to New set, with explicit Refresh offered only for the recorded set under the same recipe and detail. Framework and independently imported mapping sets are recorded immediately after each successful slot, including partial runs; a missing or unconfirmed set is never guessed from a folder or label. Mappings produced from a framework file run with that framework and are not tracked separately. Source: `src/import/stack/`, `src/settings/settings-data.ts`, `src/views/workspace-view.ts`, `src/settings/settings-tab.ts`.

### Added: implied concept notes for hierarchy levels without rows (2026-09-23)

- The nested NIST 800-53 stack recipe now gives each family its own concept note and stable CURIE, so mappings to family-level concepts can resolve. The import wizard can turn these notes on or off per folder level. Identity comes from rendered source facts, not a vault path; a source row with the same identity wins. Nested notes link to their parent concept by identity and generated parents list children, with recipe controls to disable either link family. Explicit refresh adopts older folder index notes under Replace without losing user prose; Skip leaves older notes untouched and explains the next step. When a refresh removes implied parent links, it clears only parent fields previously written by the engine, not recipe-defined or user-authored fields. Synthetic tests cover held notes, orphan accounting, identity conflicts, and recipe preservation. Source: `spec/recipe.schema.json`, `src/generation/enrich.ts`, `src/generation/generation-engine.ts`, `src/import/workbench.ts`, `recipes/import/nist-800-53-nested.json`.

- Sequential stack imports now wait for the previous framework's newly written notes to enter Obsidian's metadata cache before qualifying the next import set. If those notes remain unindexed, the stack stops with a retry action instead of guessing that the vault is empty. Source: `src/import/stack/stack-run.ts`, `src/import/stack/stack-modal.ts`.

### Added: max-detail framework stack imports (2026-09-23)

- Stack setup now chooses nested NIST 800-53 and CRI Profile recipes without changing the flat recipes proposed by ordinary file recognition. Controls and CRI upper levels become folder notes, while enhancements and diagnostic statements become nested notes. The Detail control can exclude enhancements and diagnostic statements with a run-scoped row filter; the canonical recipes stay unchanged. New stack notes record the bundled recipe ID; refresh offers require both that ID and the same recorded recipe hash, including the detail filter. Stack refresh offers are withheld for older custom IDs or ambiguous detail; start a new set unless the exact earlier recipe can be restored. NIST families become concept notes through the implied-level recipe setting described above. No licensed framework rows or prose are bundled. Source: `recipes/import/nist-800-53-nested.json`, `recipes/import/cri-profile-v2-2-nested.json`, `src/import/stack/`.

### Added: framework stack mapping imports (2026-09-23)

- Stack setup now recognizes NIST OLIR-shaped workbooks and CTID Mappings Explorer JSON in mapping slots, imports framework sets before mapping sets, and uses the existing SSSOM edge importer for a built-in NIST CSF 2.0 to 800-53 crosswalk and local publisher mappings. Its OLIR reader de-pads only the NIST 800-53 endpoint and reverses CRI workbook relationship direction when needed; its CTID reader pins ATT&CK release 16.1 and maps control-to-technique assertions to overlapping links. The completion screen shows discovered set and edge counts and unresolved endpoint actions. Reconnect explicitly refreshes the stored mapping set IDs from that open setup run, never an inferred label. CRI and CTID source rows remain local and are not bundled. Source: `src/import/stack/`, `esbuild.config.mjs`.

### Added: identity-resolved mapping endpoint links (2026-09-23)

- Crosswalk-column and SSSOM mapping imports now resolve each endpoint by its concept CURIE rather than guessing a vault path, and write folder-qualified managed `subject_note` / `object_note` wikilinks plus a one-line linked body. Missing or ambiguous concepts do not create placeholders: the edge remains, the available endpoint links, and the import summary explains the cause and what to import or repair before an explicit mapping refresh. Existing edge notes remain unchanged until refreshed. The bundled edge recipe opts in to the additive SchemaVer 1.15.0 endpoint projection convention; its target hash golden changes deliberately while recipes without these keys retain their hashes. Source: `src/generation/edge-endpoints.ts`, `src/generation/crosswalk-edge-pass.ts`, `src/import/sssom-importer.ts`, `recipes/import/crosswalk-edge.json`.

### Added: stack file recognition and framework import (2026-09-23)

- The framework stack setup now accepts source files or scans a vault folder, recognizes publisher-shaped workbook sheets and headers against framework slots, and flags an ATT&CK STIX bundle as the wrong export. Partial matches stay unfilled until the user explicitly chooses them. The review screen shows each framework's destination and defaults every run to a new import set; an existing source fingerprint can be checked for a refresh offer but never preselects it. Import runs framework slots sequentially through the recognized import path and stops at the first failure. Mapping rows were pending at this stage; the stack mapping import entry above completes that path. NIST catalog workbook display-header aliases are registry data, with canonical keys provided to its bundled recipe at import time. Source: `src/import/stack/`, `src/import/recipe-registry.ts`.
