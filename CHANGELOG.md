...
- **The JSON picker makes nesting an explicit choice.** Under a detected record chain it shows `Records inside records: ...`, then offers **Nested (N levels)** and **Only this list**; the flat **Only this list** choice remains the default until the user picks nesting. Source: `src/import/import-wizard.ts`, `src/import/detection.ts`.
- **Nested rows expose the two decisions that matter.** **Own note** offers **Folder note** or **Folders only**, and **Named by** offers **Its own identifier** or **Its place in the hierarchy**. Merge and Split are absent because nested levels are record types declared by the source, and merging them into one name would collide rather than place children into a parent's body. Source: `src/import/workbench.ts`, `src/import/mapping/view-model.ts`.
