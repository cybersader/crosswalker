...
describe('First run: synthetic framework stack', function () {
	...
	it('recognizes three synthetic sources, imports three separate sets, and offers but never selects refresh', async () => {
		...
		await browser.executeObsidian(async ({ app }, sources) => {
			...
			for (const item of sources) await app.vault.createBinary(`Sources/${item.name}`, new Uint8Array(item.bytes).buffer);
		}, payloads);
		await openStack();
		await browser.executeObsidian(() => {
		...
		try {
			await browser.waitUntil(async () => (await browser.executeObsidian(() =>
				(document.querySelector('.crosswalker-stack-modal h2')?.textContent ?? '') === 'Frameworks imported')),
		...
		expect(result.text).toContain('3 sets found in the vault');
		...
		expect(second.text).toContain('3 sets found in the vault');
		expect(second.paths).toHaveLength(6);
